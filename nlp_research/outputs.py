...
from nlp_functions import data_to_df, num_tense_inflected_verbs, calculate_idea_density, \
    ...
    incorrectly_followed_articles, number_of_unique_tokens, avg_wh_words, avg_num_nonwords, mean_similarity_of_sentences, avg_dependency_tree_height, \
...
print(f"number of tense inflected verbs: {num_tense_inflected_verbs(pipeline='en_core_web_lg', file_path="test.txt", amount=100)}")
...
#lexical diversity
print(f"manual lexical diversity: {moving_average_text_token_ratio()}")

#number of incorrectly followed articles
print(f"number of incorrectly followed articles: {incorrectly_followed_articles()}")

#number of unique tokens
print(f"number of unique tokens: {number_of_unique_tokens()}")
